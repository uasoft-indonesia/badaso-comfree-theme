<template>
  <div>
    ...
    <vs-row
      v-if="$helper.isAllowed('browse_comfree_theme_configuration') && groupList.length > 0"
    >
      <vs-col vs-lg="12">
        <vs-card>
          <vs-tabs :color="adminPanelHeaderFontColor">
            ...
            >
              <vs-row
                ...
                v-for="(config, index) in filterConfigurations('comfreeTheme')"
                :key="index"
              >
                <badaso-text
  ...
  computed: {
    groupList: {
      get() {
        return [
          {
            label: 'Comfree Theme Configuration',
            value: 'comfreeTheme'
          }
        ]
      },
